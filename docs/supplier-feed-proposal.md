...
- **Supplier feed ingestion pipeline (proof of concept, built for this proposal)** — a working prototype of the ingestion and parsing core described above: SFTP fetch with file-stability and staleness checks, checksum-based idempotent staging (re-fetching an unchanged file is a no-op, not a duplicate), and a shared per-supplier parser interface loading normalized rows into a warehouse-style staging table — with fetch and parse running as independent, separately-schedulable steps, exactly as scoped for the two-schedule design above. Verified end-to-end against a local Docker shadow stack (mock SFTP server + Postgres), including an unrecognized supplier's file being logged and skipped without affecting the rest of the batch. [github.com/gogogo-hash/SupplierFeed](https://github.com/gogogo-hash/SupplierFeed).
- **Secure API & File-Based Integration Platform — Government of British Columbia, Canada, Ministry of Finance** — Replaced a costly third-party integration platform by rebuilding and now maintaining multiple production API endpoints and 8 SFTP-based file-transfer workflows in-house (C#/.NET), exchanging structured and semi-structured data: XML, JSON, fixed-width, and PGP-encrypted files. Delivered the core migration in a 7-week window coordinating 7 cross-government teams. Maintained and contributed to building file-based data pipelines for the organization spanning +100 sources: with national credit bureaus, Insurance providers, and other government and industry partners across secure multi-zone infrastructure. Built a reusable PGP encryption/decryption pipeline and a self-healing OAuth2 authentication workflow with automated retry and failure classification, replacing manual, cron-based file handling with monitored, validated, automated delivery. Directly comparable to this SFTP ingestion, varying-format parsing, and reliability requirements in this project. Confidential government system, so no public repo, but the platform is live in production at etax.gov.bc.ca, and I'm happy to discuss any details regarding this work on a call.
